                ...
                <template v-slot:activator="{ on, attrs }">
                    <v-card outlined :color="minimaptoolbar ? null : colorToolbar" v-bind="attrs" v-on="on">
                        <v-toolbar v-if="!isDatailView" :color=colorToolbar dark hide-on-scroll height="30px" class="drag-handle">
                            ...
                            <v-toolbar-title>SomeIP SD Server Event Group Timing Config</v-toolbar-title>
                            <v-spacer></v-spacer>
                        ...
                        <v-card-text v-if="iselementOpenClose">
                            <v-text-field v-model="element.name" :label="'name  <'+element.path +'>'" :rules="rules.name" placeholder="String" style="height: 45px;" class="lable-placeholer-color"
                                        @input='inputServerName' outlined dense></v-text-field>
...
export default {
    ...
    data() {
        return {
            ...
            dialogText: false,
            editARXML: {name:'', delaymax: '', delaymin: ''},
        }
    },
    mounted () {
    ...
    methods: {
        ...
        inputServerName () {
            this.$store.commit('editServer', {compo:"Name", uuid:this.element.uuid, name:this.element.name} )
            this.$store.commit('changePathElement', {uuid:this.element.uuid, path: this.element.path, name: this.element.name} )
        ...
        saveARXML() {
            if (this.element.name != this.editARXML.name) {
                this.$store.commit('editServer', {compo:"Name", uuid:this.element.uuid, name:this.editARXML.name} )
        ...
        cancelARXML() {
            this.editARXML = {name:'', delaymax: '', delaymin: ''}
