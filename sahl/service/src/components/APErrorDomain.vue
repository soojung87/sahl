<template>
    <div :id="element.uuid">
        <v-container>
                    ...
                    <v-card outlined :color="minimaptoolbar ? null : colorToolbar" v-bind="attrs" v-on="on">
                        <v-toolbar v-if="!isDatailView" :color=colorToolbar dark hide-on-scroll height="30px" class="drag-handle">
                            <v-hover v-if="minimaptoolbar" v-slot="{ hover }">
                                <v-btn icon @click="showErrorDomain">
                            ...
                            <v-toolbar-title>AP Application Error Domain</v-toolbar-title>
                            <v-spacer></v-spacer>
            ...
            <v-dialog v-model="dialogText" persistent scrollable width="600">
                <v-card >
                    ...
                    <v-card-text>
                        ...
                        <v-row>
                            ...
                            <v-text-field v-model="editARXML.value" placeholder="int" style="height: 15px;" class="lable-placeholer-color" dense></v-text-field>
                            <label style="padding:10px;">&#60;&#47;VALUE&#62;</label>
                        </v-row>
                    </v-card-text>
                    <v-card-actions>
...
export default {
    ...
    data() {
        return {
            ...
            dialogText: false,
            editARXML: {name:'', namespace:'', value: ''},            
        }
    },
    mounted () {
    ...
    methods: {
        ...
        inputErrorDomainName () {
            this.$store.commit('editErrorDomain', {compo:"Name", uuid:this.element.uuid, name:this.element.name} )
            this.$store.commit('changePathElement', {uuid:this.element.uuid, path: this.element.path, name: this.element.name} )
        ...
        viewARXML() {
            ...
            this.editARXML.value = this.element.value
            this.dialogText= true
        },
        saveARXML() {
            if (this.element.name != this.editARXML.name) {
                this.$store.commit('editErrorDomain', {compo:"Name", uuid:this.element.uuid, name:this.editARXML.name} )
            ...
            this.element.value = this.editARXML.value
            this.cancelARXML()
        },
        cancelARXML() {
            this.editARXML = {name:'', namespace:'', value: ''}
