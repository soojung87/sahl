                    ...
                    <v-card outlined :color="minimaptoolbar ? null : colorToolbar" v-bind="attrs" v-on="on">
                        <v-toolbar v-if="!isDatailView && zoomvalue > $setZoominElement" :color=colorToolbar dark hide-on-scroll height="30px" class="drag-handle">
                            <v-hover v-if="minimaptoolbar" v-slot="{ hover }">
                                <v-btn icon @click="showSWBaseType">
                            ...
                            <v-toolbar-title>SW Base Type</v-toolbar-title>
                            <v-spacer></v-spacer>
                        ...
                        <v-card-text v-if="iselementOpenClose && zoomvalue > $setZoominElement">
                            <v-text-field v-model="element.name" :label="'name  <'+element.path +'>'" :rules="rules.name" placeholder="String" style="height: 45px;" class="lable-placeholer-color"
                                        @input='inputSWBaseTypeName' outlined dense></v-text-field>
                            ...
                            <v-text-field v-model="element.encoding" label="Base Type Encoding" placeholder="String" style="height: 45px;"  outlined dense class="lable-placeholer-color"></v-text-field>
                        </v-card-text>
            ...
            <v-dialog v-model="dialogText" persistent scrollable width="800">
                <v-card >
                    ...
                    <v-card-text>
                        ...
                        <v-row>
                            <label style="padding:10px;">&#60;BASE-TYPE-ENCODING&#62;</label>
                            <v-text-field v-model="editARXML.encoding" placeholder="String" style="height: 15px;" class="lable-placeholer-color" dense></v-text-field>
                            <label style="padding:10px;">&#60;&#47;BASE-TYPE-ENCODING&#62;</label>
                        </v-row>
                    </v-card-text>
                    <v-card-actions>
...
export default {
    ...
    data() {
        return {
            ...
            dialogText: false,
            editARXML: {name:'', category: '', encoding: ''},
        }
    },
    mounted () {
    ...
    methods: {
        ...
        inputSWBaseTypeName () {
            this.$store.commit('editSWBaseType', {compo:"Name", uuid:this.element.uuid, name:this.element.name} )
            this.$store.commit('changePathElement', {uuid:this.element.uuid, path: this.element.path, name: this.element.name} )
        ...
        viewARXML() {
            ...
            this.editARXML.encoding = this.element.encoding
            this.dialogText= true
        },
        saveARXML() {
            if (this.element.name != this.editARXML.name) {
                this.$store.commit('editSWBaseType', {compo:"Name", uuid:this.element.uuid, name:this.editARXML.name} )
            ...
            this.element.encoding = this.editARXML.encoding
            this.cancelARXML()
        },
        cancelARXML() {
            this.editARXML = {name:'', category: '', encoding: ''}
